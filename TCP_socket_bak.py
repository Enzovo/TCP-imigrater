...
class TCP_Connection_Final(TCP_Connection):
	...
	def handle_timeout(self):
		...
		print("handle_timeout allow_send_len:", allow_send_len)
	...
	def receive_packets(self, packets: List[TCP_Segment]):
		...
		for i in range(len(packets)):
			...
			print("recv info: ", len(self.receive_buffer), spare_bytes, self.RCV.WND, self.receive_buffer_start_seq)
			...
			if packets[i].ACK:
				print("recv ack: ", packets[i].SEQ + len(packets[i].data), self.RCV.NXT)
				if packets[i].SEQ + len(packets[i].data) >= self.RCV.NXT:
					self.RTO_timer.reset_timer()
					print("recv new ack reset_timer ")

			# recv new bytes
			if packets[i].SEQ + len(packets[i].data) > self.RCV.NXT:
				recv_new_bytes = True
			print("packets[",i,"].SEQ: ",packets[i].SEQ,"self.RCV.NXT: ",self.RCV.NXT)
		...
		for i in range(len(self.receive_buffer)):
			if self.receive_buffer[i] == None:
				...
				break
		if not seeNone:
			self.RCV.NXT = len(self.receive_buffer) + self.receive_buffer_start_seq
		
		# update RCV.WND
		# TODO delete 8192 use name
		#FIXME: this one should be 0
		self.RCV.WND = len(self.receive_buffer) - (self.RCV.NXT - self.receive_buffer_start_seq)
		
		print("self.receive NXT start_seq self.RCV.WND : ", self.RCV.NXT, self.receive_buffer_start_seq, self.RCV.WND)
